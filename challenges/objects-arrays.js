...
graduates.forEach(institues => {
  return universities.push(institues.university);
});
...
graduates.forEach(person => {
  return contactInfo.push(person.first_name, person.email);
});

console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter(x => x.university.includes("Uni"));
...
const lowCaseAnimalNames = zooAnimals.map(converter => {
  return converter.animal_name.toLowerCase();
});
...
const populationTotal = zooAnimals.reduce(
  (populationTotal, populationNumber) =>
    populationTotal + populationNumber.population,
  0
);
